...
class PokeUserJoin(db.Model):
    pokecollab_id = db.Column(db.Integer, db.ForeignKey('pokecollab.id'), primary_key=True)
...
class pokecollab(db.Model):
    __tablename__ = 'pokecollab'
    id = db.Column(db.Integer, primary_key=True)
    ...
    def exists(name):
         return pokecollab.query.filter_by(name=name).first()
